#!/usr/bin/env python3
"""
构建独立 Debian/Linux 可执行文件的打包脚本。
- 依赖：本机已有 Python 3.10.x（建议 3.10.13/3.10.14）
- 生成产物：dist/<name> (单文件可执行)
使用示例：
    python3 debian_build_binary.py --entry app.py --name web-scrcpy
    python3 debian_build_binary.py --entry app.py --name web-scrcpy --extra --add-data static:static --add-data templates:templates
"""
...
PYINSTALLER_VERSION = "5.13.2"  # 支持 Python 3.10，Linux 稳定
...
def build(python_bin: Path, entry: Path, name: str, extra_args: list[str]):
    # 清理旧构建
...
def main():
    parser = argparse.ArgumentParser(description="Build Debian/Linux binary with PyInstaller")
    ...
    build(python_bin, entry, args.name, extra_args)

    dist_bin = project_root / "dist" / args.name
    print(f"\n✅ 打包完成，可执行文件：{dist_bin}")
    print("复制 dist/<name> 到无 Python 的 Debian/Linux 系统即可运行。")

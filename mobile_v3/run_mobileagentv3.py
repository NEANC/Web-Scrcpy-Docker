...
def run_instruction(
    ...
    # Debug: show which adb/hdc path is used (includes -s <serial> for adb)
    try:
        path_desc = adb_path if adb_path else hdc_path
        if path_desc:
            print(f"[Agent] Controller path: {path_desc}")
    except Exception:
        pass
    ...
    for step in range(max_step):
        if _should_stop():
            print("[Agent] Stop signal received. Exiting.")
            break
        if step == max_step:
        ...
        for _ in range(5):
            if _should_stop():
                print("[Agent] Stop during screenshot acquisition.")
                break
...
{action_description}'''
            ...
            if _should_stop():
                print("[Agent] Stop before handling operator action.")
                break
            ...
            if action_object['action'] == "answer":
                ...
                with open(task_result_path, 'w', encoding='utf-8') as json_file:
                    json.dump(task_result_data, json_file, ensure_ascii=False, indent=4)
                final_answer_content = answer_content
                break
            ...
            if _should_stop():
                print("[Agent] Stop before executing device action.")
                break
        ...
        if step == 0:
            for _ in range(8):
                if _should_stop():
                    break
                time.sleep(1) # maybe a pop-up when first open an app
        for _ in range(2):
            if _should_stop():
                break
            time.sleep(1)
        ...
        for _ in range(5):
            if _should_stop():
                print("[Agent] Stop during post-action screenshot acquisition.")
                break
